...
namespace WCP2dToy{
/** * @brief Class responsible for solving matrix equations related to charge distribution in GeomCells and GeomWires.
 *
 * This class encapsulates the logic for solving linear algebra problems involving charges distributed across geometric cells and wires,
 * utilizing both direct and L1 regularization methods. It manages mappings between cells and wires, computes chi-squared values,
 * and handles the inversion of matrices required for solving the problem.
 *
 * @details The solver uses Eigen libraries for handling vector and matrix operations efficiently. It maintains internal state variables
 * such as chi-squared values, degrees of freedom, and flags indicating the status of the solution process. Additionally, it provides
 * interfaces to retrieve detailed information about the solution, including individual weights and contributions from cells and wires.
 *
 * @note The class assumes the existence of certain data structures defined within the WCP namespace, such as GeomCell, GeomWire,
 * CellIndexMap, and WireIndexMap, which are essential for mapping and indexing purposes during the solution process.
