...
/** * @brief Implements a memory-efficient tiling strategy for processing geometric data in a liquid argon time projection chamber (LArTPC).
 *
 * This class extends the functionality of `WCP::TilingBase` to handle large datasets efficiently by managing wire-cell relationships and merging operations within specified time slices. It supports various methods for initializing, updating, and querying the state of wires and cells, including handling bad merges, forming fired merges with optional charge information, and performing deghosting operations to improve signal clarity.
