...
/**
 * @class WCP2dToy
 * @brief This class contains methods for clustering live and dead clusters.
  
  * @fn Clustering_live_dead
 * @brief This function performs clustering of live and dead clusters based on certain conditions.
 * @param live_clusters A reference to a selection of 3D clusters.
 * @param dead_clusters A reference to a selection of 3D clusters.
 * @param cluster_length_map A reference to a map of cluster lengths.
 * @param cluster_connected_dead A reference to a set of connected dead clusters.
  ...
  for (auto it = dead_live_cluster_mapping.begin(); it!= dead_live_cluster_mapping.end(); it++){
	...
	cluster_connected_dead.insert(cluster_1);
		...
		if (fabs(angle2-3.1415926/2.)<7.5/180.*3.1415926 &&
		  ...
		  flag_para = true;
		  if (WCP2dToy::is_angle_consistent(dir1,dir2,false,15,angle_u,angle_v,angle_w,3) && WCP2dToy::is_angle_consistent(dir3,dir2,true,15,angle_u,angle_v,angle_w,3))
		    flag_merge = true;
		}else{
		  bool flag_const1 = WCP2dToy::is_angle_consistent(dir1,dir2,false,10,angle_u,angle_v,angle_w);
		  bool flag_const2 = WCP2dToy::is_angle_consistent(dir3,dir2,true,10,angle_u,angle_v,angle_w);

		  if (flag_const1 && flag_const2){
		    flag_merge = true;
		  }else if (flag_const1 && length_2 < 6*units::cm && length_1 > 15*units::cm){
		    if (WCP2dToy::is_angle_consistent(dir1,dir2,false,5,angle_u,angle_v,angle_w,3))
		      flag_merge = true;
		  }else if (flag_const2 && length_1 < 6*units::cm && length_2 > 15*units::cm){
		    if (WCP2dToy::is_angle_consistent(dir3,dir2,true,5,angle_u,angle_v,angle_w,3))
		       flag_merge = true;
		  }
...
// bool WCP2dToy::IsCrossing(WCP::Point& p, double theta, double phi, WCP::SlimMergeGeomCell *mcell, WCPSst::GeomDataSource& gds){
