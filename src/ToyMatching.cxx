...
/** * Converts a point in Cartesian coordinates to a voxel ID within a 3D grid.
 *
 * This method calculates the voxel indices along the x, y, and z axes based on the input point's coordinates,
 * applying rounding and clamping operations to ensure they fall within valid bounds. The final voxel ID is computed
 * using a formula that encodes these three indices into a single integer value suitable for indexing into a flattened
 * representation of the 3D grid.
 *
 * @param p Reference to a Point object representing the Cartesian coordinates of the point to be converted.
 * @return Integer representing the unique voxel ID corresponding to the input point's position within the grid.
...
/** * @brief Matches optical flashes to TPC clusters using a combination of statistical methods and optimization techniques.
 *
 * This function performs the following steps:
 * - Initializes parameters and constants required for the matching process.
 * - Iterates over all possible combinations of flashes and clusters to create initial bundles.
 * - Evaluates and filters these bundles based on statistical criteria such as KS distance, Chi-squared value, and degrees of freedom.
 * - Further refines the selection of bundles by applying additional checks and removing inconsistent matches.
 * - Uses a lasso regression model to optimize the assignment of flashes to clusters, considering penalties for unused flashes and multiple assignments of clusters.
 * - Organizes the final set of matched bundles and returns them.
 *
 * @param eventTime Event timestamp.
 * @param time_offset Offset applied to the event time.
 * @param nrebin Number of rebinning factors.
 * @param pl Pointer to the photon library containing PMT-related data.
 * @param group_clusters Map of main clusters to their associated clusters.
 * @param flashes Selection of opflashes to match against clusters.
 * @param run_no Run number identifier.
 * @param flag_data Flag indicating whether the data is real or simulated.
 * @param flag_add_light_yield_err Flag to include additional light yield error.
 * @param flag_timestamp Flag to consider timestamps during matching.
 * @return FlashTPCBundleSelection Final set of matched bundles between flashes and clusters.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Organizes matched bundles based on their association with opflashes and performs hierarchical clustering.
 *
 * This method processes a collection of FlashTPCBundle objects, grouping them according to their associated opflashes.
 * It then evaluates these groups to identify the most probable bundle for each flash, merging compatible bundles,
 * and categorizing unmatched bundles for further processing.
 *
 * @param results_bundles Reference to a selection of FlashTPCBundle objects to be organized and processed.
 * @param cos_pe_low Pointer to an array containing lower threshold values for photoelectron counts used in bundle comparison.
 * @param cos_pe_mid Pointer to an array containing middle threshold values for photoelectron counts used in bundle comparison.
 * @param fc_bundles_map Map associating pairs of Opflash and PR3DCluster pointers to corresponding FlashTPCBundle objects.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
