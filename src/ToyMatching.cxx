...
/** * Converts a point in Cartesian coordinates to a voxel ID within a specified grid.
 *
 * The conversion takes into account the position of the point relative to the origin
 * of the grid and maps it to a unique voxel identifier. The grid dimensions and offsets
 * are defined by constants embedded within the function.
 *
 * @param p Reference to a Point object representing the Cartesian coordinates.
 * @return Integer value representing the voxel ID.
 ...
 * - Initializes parameters and mappings from the photon library.
 * - Defines constants related to photoelectron counts.
 * - Iterates over optical flashes and TPC clusters to create initial matches encapsulated in `FlashTPCBundle` objects.
 * - Evaluates and filters these matches based on statistical criteria such as KS distance, chi-squared value, and degrees of freedom.
 * - Further refines matches by applying consistency checks and removing inconsistent bundles.
 * - Uses a lasso regression model to regularize and optimize the match strengths.
 * - Organizes and returns the final set of matched bundles.
 *
 * @param eventTime Event timestamp.
 * @param time_offset Offset applied to flash times.
 * @param nrebin Number of rebinning factors.
 * @param pl Pointer to the photon library containing calibration data.
 * @param group_clusters Map of main clusters to additional clusters.
 * @param flashes Collection of optical flashes.
 * @param run_no Run number identifier.
 * @param flag_data Flag indicating whether real data is being processed.
 * @param flag_add_light_yield_err Flag to include light yield error in calculations.
 * @param flag_timestamp Flag to consider timestamps in matching process.
 * @return FlashTPCBundleSelection Final collection of matched bundles after filtering and optimization.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.2.
*/ 
...
/** * Organizes matched bundles based on flash associations and quality criteria.
 *
 * This method processes a collection of FlashTPCBundle objects, grouping them by their associated Opflash object.
 * It then evaluates these groups to identify the most suitable bundle for each flash, considering factors such as KS distance,
 * chi-squared value, and proximity to PMTs. Bundles deemed less likely are either merged into better-suited ones or placed
 * into secondary rounds for further evaluation. Ultimately, the method refines the initial set of bundles, ensuring that each
 * flash is associated with the optimal cluster configuration.
 *
 * @param results_bundles Reference to a selection of FlashTPCBundle objects to be organized.
 * @param cos_pe_low Pointer to an array containing low-energy PE cosine values used in bundle ranking.
 * @param cos_pe_mid Pointer to an array containing mid-energy PE cosine values used in bundle ranking.
 * @param fc_bundles_map Map associating pairs of Opflash and PR3DCluster pointers to their corresponding FlashTPCBundle objects.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.2.
*/ 
