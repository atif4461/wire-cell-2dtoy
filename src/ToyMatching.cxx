...
/** * Converts a 3D point coordinate to a voxel ID within a defined grid.
 *
 * The conversion maps the x, y, and z coordinates of a point to corresponding
 * voxel indices in a 3D grid. The grid dimensions and offsets are specified,
 * ensuring that the resulting voxel IDs fall within valid bounds.
 *
 * @param p Reference to a Point object containing the x, y, and z coordinates.
 * @return Integer representing the voxel ID.
...
/** * \brief Perform many-to-many charge-light matching between TPC clusters and PMT flashes.
 *
 * This function implements a sophisticated matching algorithm to associate TPC charge clusters with PMT light flashes,
 * taking into account various factors such as timing offsets, rebinning, and light yield errors. It uses a photon library
 * to simulate expected light patterns and applies statistical tests to validate matches. The process involves creating
 * potential bundles of clusters and flashes, evaluating them against criteria, and selecting the best matches using
 * compressed sensing techniques.
 *
 * \param eventTime Event timestamp used for calculations.
 * \param time_offset Offset applied to adjust the timing of TPC clusters.
 * \param nrebin Number of rebins for time slicing.
 * \param pl Pointer to the Photon_Library object containing simulation parameters and libraries.
 * \param group_clusters Map of main TPC clusters to additional clusters.
 * \param flashes Collection of PMT flashes to be matched.
 * \param run_no Run number identifier.
 * \param flag_data Boolean indicating whether real data is being processed.
 * \param flag_add_light_yield_err Flag to include additional light yield error in calculations.
 * \param flag_timestamp Flag to consider timestamps in the matching process.
 * \return Selection of matched FlashTPCBundle objects representing successful associations.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.2.
*/ 
...
/** * Organizes matched bundles based on flash associations and quality metrics.
 *
 * @param results_bundles Reference to a selection of FlashTPCBundle objects to be organized.
 * @param cos_pe_low Pointer to an array containing low threshold cosine values for PE calculation.
 * @param cos_pe_mid Pointer to an array containing mid threshold cosine values for PE calculation.
 * @param fc_bundles_map Map associating pairs of Opflash and PR3DCluster pointers to FlashTPCBundle pointers.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.2.
*/ 
