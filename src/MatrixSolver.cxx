 ...
 *
 * Initializes the matrix solver with the given geometric cell selection, wire selection,
 * cell-wire map, wire-cell map, wire charge map, and wire charge error map.
 *
 * @param allmcell Geometric cell selection.
 * @param mwires Wire selection.
 ...
 * @brief Solves the matrix equation using the constructed matrices and vectors.
 *
 * Fills in the matrix contents, constructs the vector, and solves the equation.
  

 * @brief Constructs the matrix MA.
 *
 * Iterates over the wire selection and fills in the matrix MA.
 
for (int j=0;j!=mwires.size();j++){
 * @brief Constructs the matrix MB.
 *
 * Iterates over the wire selection and fills in the matrix MB.
 
for (int k=0; k!=wire_cell_map[mwires[j]].size();k++){
 * @brief Transposes the matrix MB.
 *
 * Computes the transpose of the matrix MB.
 
MBT->Transpose(*MB);
 * @brief Transposes the matrix MA.
 *
 * Computes the transpose of the matrix MA.
 
MAT->Transpose(*MA);
 * @brief Computes the product of MB, Vy, and MBT.
 *
 * Calculates the product of the matrices MB, Vy, and MBT.
 
*VBy = (*MB) * (*Vy) * (*MBT);
 * @brief Inverts the matrix VBy.
 *
 * Computes the inverse of the matrix VBy.
 
VBy_inv->Invert();
 * @brief Computes the product of MAT, VBy_inv, and MA.
 *
 * Calculates the product of the matrices MAT, VBy_inv, and MA.
 
*MC = (*MAT) * (*VBy_inv) * (*MA);
 * @brief Regularizes the solution.
 *
 * Adds a regularization term to the solution.
 
lambda = 3./total_wire_charge/2.*scale_factor;
 * @brief Decomposes the matrix VBy_inv.
 *
 * Performs a Cholesky decomposition of the matrix VBy_inv.
 
TDecompChol test(*VBy_inv);
test.Decompose();
 * @brief Fills in the results.
 *
 * Fills in the solution vectors and matrices.
...
/**
 * @brief Solves the L1 regularization problem using the Lasso model.
 *
 ...
 *
 * @return A collection of geometric cells.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
...
/**
 * Returns the charge associated with a merged geometry cell.
 *
 * @param mcell pointer to the MergeGeomCell object
 * @return the charge value of the cell
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
...
/**
 * @brief Retrieves the index associated with a given GeomWire in the matrix solver.
 *
 * @param wire Pointer to the GeomWire object.
 * @return Index associated with the GeomWire if found, -1 otherwise.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
...
/**
 * @brief Returns the W value at the specified index if it is within bounds.
 *
 * @param index The index at which to retrieve the W value.
 * @return The W value at the specified index, or 0 if out of bounds.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
...
/**
 * Returns the value of the G matrix at the specified indices.
 *
 * @param index_mw Index of the mw dimension.
 * @param index_mc Index of the mc dimension.
 * @return The value of the G matrix at the specified indices, or 0 if the indices are out of bounds.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
...
/**
 * @brief Destructor for MatrixSolver class
 * 
 * Releases dynamically allocated memory for matrices Wy, Cx, dCx, MWy_pred, and MWy
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
