...
/** * @brief Constructor for LowmemTiling class.
 *
 * Initializes the LowmemTiling object with specified parameters and retrieves wire information from the geometry data source.
 *
 * @param time_slice Time slice identifier.
 * @param gds Reference to the geometry data source containing wire plane information.
 * @param holder Reference to the WCPHolder object used for storing processed data.
...
/** * @brief Constructor for LowmemTiling class initializing tiling parameters and wire selections.
 *
 * Initializes the LowmemTiling object with specified time slice, rebinning factor,
 * geometry data source, and event holder. Retrieves wire selections for planes U, V, and W.
 *
 * @param time_slice Time slice duration for binning events.
 * @param nrebin Number of bins per original bin after rebining.
 * @param gds Reference to the geometry data source containing wire information.
 * @param holder Reference to the event holder managing event data.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Finds the V-plane wires that correspond to the specified range of Y-plane wires in cells marked as good.
 *
 * This function iterates over a collection of geometry cells (`two_good_wire_cells`), checking each cell for Y-plane wires within a defined index range (2336 to 2463).
 * If such wires are found, it collects all associated V-plane wires from those cells into a set to ensure uniqueness.
 * Finally, it converts the set back into a `GeomWireSelection` object containing the selected V-plane wires.
 *
 * @return A `GeomWireSelection` object containing the V-plane wires corresponding to the specified Y-plane wire indices in good cells.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Divides wires into smaller segments based on specified limits and conditions.
 *
 * This method iterates over parent wires, sorts their associated wires,
 * identifies segments that meet size criteria, creates new wires and cells,
 * and updates mappings accordingly.
 *
 * @param wire_limit Maximum number of wires allowed per segment.
 * @param min_wire Minimum length required for a wire segment.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Merges wires within the LowmemTiling framework.
 *
 * This function implements the merging of wires based on the original mergetoytiling algorithm.
 * It iterates through parent wire maps, checking if the parent wire is valid before proceeding with the merge operation.
 * The merging process continues until no more merges can be performed.
 * After merging, it calculates the merged wire charges and orders the wires within cells.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Calculates the merged wire charges and their associated errors for all wire cells.
 *
 * This method iterates over each wire cell in the `wire_cells_map`, calculates the total charge
 * and its uncertainty for each merged wire, and stores these values in `wirechargemap` and
 * `wirecharge_errmap`. The calculation considers both good and bad wires, applying appropriate
 * uncertainties based on the wire type and plane.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Further merges wires by attempting to add each wire from the input selection to existing merged wires.
 * If a wire can be added to an existing merged wire, it replaces the original wire with the updated merged wire.
 * Otherwise, it adds the wire as a new entry in the output selection.
 *
 * @param allwire Reference to a collection of geometry wires to be processed and potentially merged.
 * @return The difference between the initial number of wires and the final number after merging.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Replaces an existing wire with a new wire in various mapping structures.
 *
 * This function replaces occurrences of `old_wire` with `new_wire` across multiple internal mappings,
 * ensuring that references to `old_wire` are updated to reference `new_wire`. It handles updates
 * in wire type maps, wire-parent wire maps, and cell-wire maps. After updating the mappings, it deletes
 * the `old_wire`.
 *
 * @param old_wire Pointer to the wire object to be replaced.
 * @param wire Pointer to the new wire object that will replace the old wire.
...
/** * @brief Removes a wire and associated mappings from various internal data structures.
 *
 * This function removes a specified wire from multiple internal mapping structures,
 * including cell-wire maps, wire-wire maps, wire charge maps, wire charge error maps,
 * and wire type maps. It ensures that all references to the wire are properly cleaned up.
 *
 * @param wire Pointer to the MergeGeomWire object representing the wire to be removed.
 * @return Always returns true indicating successful removal.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Removes a wire from various internal mappings and deletes the wire object.
 *
 * This function removes the specified wire from multiple internal mappings including
 * wire-cells mapping, wire-parent wire mapping, parent wire-wires mapping,
 * wire charge mapping, wire charge error mapping, and wire type mapping. It checks
 * if the wire has associated cells before removing it from the wire-cells mapping.
 * If the wire is successfully removed from all mappings, the wire object itself is deleted.
 *
 * @param wire Pointer to the MergeGeomWire object to be removed.
 * @return True if the wire was successfully removed and deleted, false otherwise.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Erases a specified cell from various internal collections and maps within the LowmemTiling class.
 *
 * This function removes the given cell from multiple vectors that store cells categorized by the number of good wires associated with them,
 * specifically `three_good_wire_cells`, `two_good_wire_cells`, and `one_good_wire_cells`. It then proceeds to update the mapping between cells and their corresponding wires stored in `cell_wires_map`.
 * For each wire connected to the cell, the cell is removed from the list of cells associated with that wire in `wire_cells_map`. If removing the cell leaves a wire without any connections, the wire itself is removed via the `remove_wire` function.
 * Finally, the entry for the cell in `cell_wires_map` is erased.
 *
 * @param cell Pointer to the SlimMergeGeomCell object to be erased.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Removes a specified geometry cell from the internal maps and deletes the cell object.
 *
 * This function removes the given cell from the `cell_wires_map` and updates the `wire_cells_map`
 * by removing references to the cell from any associated wires. If a wire becomes disconnected from
 * all cells, it is also removed. Finally, the cell object itself is deleted.
 *
 * @param cell Pointer to the SlimMergeGeomCell that needs to be removed.
...
/** * Tests a point within a specified region in a Low Memory Tiling scheme.
 *
 * This function checks whether a point lies within a bounding box defined by
 * `bmin_w` and `bmax_w` along the w-axis, considering the crossing points of wires
 * at various offsets around the given point `(dis_u, dis_v)`.
 *
 * @param pcell Reference to a vector that stores valid points passing the criteria.
 * @param dis_u The u-coordinate of the center point.
 * @param dis_v The v-coordinate of the center point.
 * @param bmin_w Minimum value of the w-coordinate for the bounding box.
 * @param bmax_w Maximum value of the w-coordinate for the bounding box.
 * @param u_pitch Pitch distance between consecutive u-wires.
 * @param v_pitch Pitch distance between consecutive v-wires.
 * @param dis1 Output parameter storing the wire distance for the central point.
 * @return True if any valid point is found within the bounding box, otherwise False.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Tests crossing points between wires in specified planes within a defined range.
 *
 * This method calculates potential crossing points between wires in two perpendicular planes,
 * considering a specified distance threshold and wire geometry constraints. It iterates over
 * positions along one wire plane, checking for intersections with another plane and evaluating
 * whether these crossings fall within predefined bounds.
 *
 * @param pcell Reference to a vector that stores valid crossing points found during the process.
 * @param dis Distance parameter used in crossing point calculations.
 * @param plane Wire plane type (either kUwire or kVwire).
 * @param low_limit Lower bound of the position range to be considered.
 * @param high_limit Upper bound of the position range to be considered.
 * @param pitch Pitch value for the first wire plane.
 * @param pitch1 Pitch value for the second wire plane.
 * @param w_pitch Width pitch value for the wire geometry.
 * @param wire1 Pointer to the starting wire object in the specified plane.
 * @param wire2 Pointer to the ending wire object in the specified plane.
 * @param dir Direction of iteration (1 for forward, -1 for backward).
 * @param bmin_w Minimum allowed distance for crossing points.
 * @param bmax_w Maximum allowed distance for crossing points.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Prints the sizes of various maps used in LowmemTiling.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Re-establishes various mapping relationships within the LowmemTiling class.
 *
 * This function reconstructs mappings between geometry cells and their associated wires,
 * including up-wires, VP-wires, and WP-wires. It first creates temporary maps to store these
 * relationships, then clears existing wire-related structures, and finally repopulates them
 * with newly created merge geometries and updated mappings.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Perform local deghosting on geometry cells.
 *
 * This method processes geometry cells to remove ghost cells that do not meet certain criteria related to their adjacency and charge distribution across wire planes.
 *
 * @param good_mcells Set of geometry cells considered as good cells.
 * @param map_mcell_charge Map associating each geometry cell with its corresponding charge value.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Perform local deghosting on geometry cells.
 *
 * This function processes sets of geometric cells to identify and remove ghost cells,
 * which are cells that do not correspond to actual physical interactions but appear due to noise or other artifacts.
 * It uses information about adjacent cells and shared wires to make decisions about which cells to keep or discard.
 *
 * @param potential_good_mcells Set of potentially good geometry cells that need evaluation.
 * @param good_mcells Set of confirmed good geometry cells.
 * @param flag_del Flag indicating whether identified ghost cells should be physically removed from the dataset.
 * @return Selection of geometry cells marked as ghosts after deghosting process.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Resets the internal state of the LowmemTiling object by clearing various containers that store information about fired wires,
 * bad wires, good wire cells, and mappings between wires and cells. This includes cleaning up vectors and maps related to wire charges,
 * wire types, and cell-wire relationships.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Initializes good cells with charge information from fired wires.
 *
 * This function forms fired merge wires with charge and iterates through
 * combinations of fired wires in U, V, and W planes to create slim merged geometry cells.
 * Each cell is set with the current time slice and added to the holder if successfully created.
 *
 * @param u_time_chs Map of time channels to wire indices in the U plane with associated charges.
 * @param v_time_chs Map of time channels to wire indices in the V plane with associated charges.
 * @param w_time_chs Map of time channels to wire indices in the W plane with associated charges.
 * @param time_ch_charge_map Map of wire index pairs to their corresponding charges.
 * @param time_ch_charge_err_map Map of wire index pairs to their corresponding charge errors.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Initializes good cells by forming fired merge wires and creating slim merge geometry cells.
 *
 * This function initializes good cells within the LowmemTiling class by first forming fired merge wires
 * from the provided maps of U, V, and W time channels. It then iterates over these formed wires,
 * creates slim merge geometry cells for combinations of U, V, and W wires, sets their time slice,
 * and adds them to the cell holder.
 *
 * @param u_time_chs Map containing U wire channel information indexed by time slices.
 * @param v_time_chs Map containing V wire channel information indexed by time slices.
 * @param w_time_chs Map containing W wire channel information indexed by time slices.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Initializes good cells by forming fired merge wires and creating slim geometry cells with various combinations of good and bad wires.
 *
 * This method processes slices and their errors to identify good wires and then creates cells consisting of these wires. It handles both scenarios where all planes have good wires (U/V/W = 1/1/1) and cases where one plane has a bad wire while others have good ones (U/V/W = 1/1/0, U/V/W = 1/0/1, U/V/W = 0/1/1).
 *
 * @param slice The input slice containing wire information.
 * @param slice_err The error slice associated with the input slice.
...
/** * @brief Retrieves all cell centers within the tiling structure.
 *
 * This method calculates the center of each cell defined in the tiling structure
 * by finding the crossing point between the middle wires of the u and v wire sets
 * associated with each cell. The resulting points are returned as a vector of points.
 *
 * @return WCP::PointVector A vector containing the center points of all cells.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Retrieves all good wires from the wire cells map.
 *
 * This function iterates through the wire_cells_map and collects wires that are marked as good according to the wire_type_map.
 *
 * @return GeomWireSelection containing all good wires.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Retrieves all bad wires from the wire cells map.
 *
 * This function iterates through the wire_cells_map and collects wires that are marked as bad in the wire_type_map.
 *
 * @return GeomWireSelection A collection of bad wires.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Creates good wire cells from identified fired wires in different planes.
 *
 * This function identifies all fired wires across multiple planes (U, V, W) from both sets of good wire cells
 * (three_good_wire_cells and two_good_wire_cells). It then determines leftover wires that were not included in these sets,
 * creates new merged wire objects for these leftovers, and constructs geometry cells based on combinations of these wires
 * while marking certain planes as bad where applicable. Finally, it checks for overlaps between newly created cells and
 * pre-existing good cells, adding those without overlap to the set of one_good_wire_cells.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Creates single geometry cells from merged wire cells.
 *
 * This function iterates over a map of cell wires, creating individual geometry cells
 * from each merged wire cell. It accumulates these cells into a collection and records
 * the center point of each set of created cells.
 *
 * @return GeomCellSelection A collection of newly created geometry cells.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Creates single geometric cells from a slim merged geometry cell.
 *
 * This function generates individual geometric cells based on the wires present in the provided slim merged geometry cell.
 * It calculates crossing points between different sets of wires and determines the boundaries of the cells.
 * Cells that meet certain criteria (such as containing their center within the detector volume) are added to the output collection.
 *
 * @param mcell Pointer to the SlimMergeGeomCell object containing the wires used to create the cells.
 * @return GeomCellSelection Collection of created geometric cells.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Checks for bad cells in the specified planes and updates the internal state accordingly.
 *
 * This function examines the time slices associated with chirps in the u-plane, v-plane, and w-plane maps.
 * It determines whether regeneration of bad cells is necessary based on the current time slice compared to the previous one.
 * If regeneration is required, it initializes the bad cells; otherwise, it copies the existing bad cells and wires.
 *
 * @param tiling Pointer to the LowmemTiling object containing the current state and geometry information.
 * @param uplane_map Reference to the ChirpMap representing the u-plane chirps.
 * @param vplane_map Reference to the ChirpMap representing the v-plane chirps.
 * @param wplane_map Reference to the ChirpMap representing the w-plane chirps.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Initializes bad cells in the LowmemTiling object by forming groups of bad wires
 * and creating special bad wire cells.
 *
 * @param uplane_map Reference to the ChirpMap representing the u-plane.
 * @param vplane_map Reference to the ChirpMap representing the v-plane.
 * @param wplane_map Reference to the ChirpMap representing the w-plane.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Forms two bad cells by identifying crossing points between pairs of wires in different planes and creating geometric cells around these crossings.
 *
 * This function iterates over sets of bad wires in the U, V, and W planes to find intersections that define boundaries of potential problematic regions within the detector geometry. It calculates distances between wires, checks for crossing points, and constructs cells that encompass these intersections. These cells are marked as having issues in specific wire planes and are stored for further processing.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * Forms merged wires with associated charge information for specified time slices across U, V, and W planes.
 *
 * This function processes sets of channels active within a particular time slice for each of the three wire planes (U, V, W)
 * in a Liquid Argon Time Projection Chamber (LArTPC). It merges consecutive wires that belong to the same channel set,
 * assigns them their respective charge values from provided maps, and stores these merged wires in corresponding vectors.
 *
 * @param u_time_chs Map associating time slices with sets of active channels in the U plane.
 * @param v_time_chs Map associating time slices with sets of active channels in the V plane.
 * @param w_time_chs Map associating time slices with sets of active channels in the W plane.
 * @param time_ch_charge_map Map storing charge values for each combination of time slice and channel.
 * @param time_ch_charge_err_map Map storing error values for charge measurements corresponding to each combination of time slice and channel.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Forms merged wires for fired channels in U, V, and W planes within a specified time slice.
 *
 * This method processes the fired channels in the U, V, and W planes for a given time slice,
 * merging consecutive wires into single MergeGeomWire objects. The resulting merged wires are stored
 * in respective vectors for each plane.
 *
 * @param u_time_chs Map containing sets of channel numbers for the U plane indexed by time slices.
 * @param v_time_chs Map containing sets of channel numbers for the V plane indexed by time slices.
 * @param w_time_chs Map containing sets of channel numbers for the W plane indexed by time slices.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Forms merged wires by firing mechanism from slices with error handling.
 *
 * This method processes two input slices containing channel groups and their corresponding errors,
 * mapping charges to geometric wires. It then merges consecutive wires within each plane type (U, V, W).
 *
 * @param slice The primary slice containing channel segments and associated charges.
 * @param slice_err The secondary slice containing channel segments and associated charge errors.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Forms bad merge wires for the specified planes within a given time slice.
 *
 * This method processes the chirp maps for the U, V, and W planes to identify and group wires that fall within a specified time slice.
 * If wires are contiguous, they are added to the same MergeGeomWire object; otherwise, new MergeGeomWire objects are created.
 * Identified bad wires are stored in respective vectors for each plane.
 *
 * @param uplane_map Reference to the ChirpMap representing the U-plane.
 * @param vplane_map Reference to the ChirpMap representing the V-plane.
 * @param wplane_map Reference to the ChirpMap representing the W-plane.
...
/** * Destructor for LowmemTiling class.
 * Cleans up dynamically allocated memory associated with wire type map and cell wires map.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Retrieve wire selections associated with a geometric cell.
 *
 * This function returns the set of wires that are part of the specified geometric cell.
 *
 * @param cell The geometric cell for which to retrieve the associated wires.
 * @return A selection of wires associated with the given cell.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
...
/** * @brief Retrieve the geometric cell selections associated with a specified wire.
 *
 * This function returns the set of geometric cells that are linked to the provided wire within the tiling structure.
 *
 * @param wire Reference to the wire object whose associated cells are to be retrieved.
 * @return GeomCellSelection Collection of geometric cells associated with the specified wire.
 * This comment was generated by Qwen/Qwen2.5-Coder-32B-Instruct:None at temperature 0.01.
*/ 
