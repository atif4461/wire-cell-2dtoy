 ...
 *
 * Checks if the number of command line arguments is less than 4.
 * If true, prints usage message and returns 1.
...
cerr << "Extent: "
     ...
     << " z:" << ex[2]/units::m << " m"
     << endl;
 * @brief Performs simulation and analysis.
 *
 * Loops through events, applies reconstruction algorithms, and saves results.
 
for (int i=start_num;i!=end_num+1;i++){
         * @brief Jumps to specific event.
     
    sds.jump(i);
    WCP::Slice slice = sds.get();

         * @brief Creates ToyTiling object.
     *
     * @param slice Event slice.
    ...
    toytiling[i] = new WCP2dToy::ToyTiling(slice,gds,0.15,0.2,0.1,threshold_ug,threshold_vg, threshold_wg, &uplane_rms, &vplane_rms, &wplane_rms);
         ...
         * @brief Applies two-plane reconstruction algorithm.
     *
     * @param i Event index.
     * @param nrbin Rebin factor.
     * @param gds Geometry data source.
     * @param uplane_rms U-plane RMS values.
     * @param vplane_rms V-plane RMS values.
     * @param wplane_rms W-plane RMS values.
     * @param uplane_map U-plane map.
     * @param vplane_map V-plane map.
     * @param wplane_map W-plane map.
     
    if (two_plane)
      toytiling[i]->twoplane_tiling(i,nrebin,gds,uplane_rms,vplane_rms,wplane_rms, uplane_map, vplane_map, wplane_map);

         * @brief Retrieves all cells and wires from ToyTiling object.
     *
    ...
    GeomWireSelection allwire = toytiling[i]->get_allwire();
         ...
         * @brief Creates MergeToyTiling object.
     *
     * @param toytiling ToyTiling object.
     * @param i Event index.
    ...
    mergetiling[i] = new WCP2dToy::MergeToyTiling(*toytiling[i],i,3);
         ...
         * @brief De-ghosts cells if two-plane reconstruction is enabled.
     
    if (two_plane) 
      mergetiling[i]->deghost();

         * @brief Retrieves all merged cells and wires from MergeToyTiling object.
     *
     * @return Merged cell selection and merged wire selection.
     
    GeomCellSelection allmcell = mergetiling[i]->get_allcell();
    GeomWireSelection allmwire = mergetiling[i]->get_allwire();

         * @brief Creates ToyMatrix object.
     *
     * @param toytiling ToyTiling object.
    ...
    toymatrix[i] = new WCP2dToy::ToyMatrix(*toytiling[i],*mergetiling[i]);
         ...
         * @brief Iterates and solves ToyMatrix object.
     
    if (toymatrix[i]->Get_Solve_Flag()==0)
      WCP2dToy::ToyMatrixIterate toymatrix_it(*toymatrix[i],2000,1e5);

         * @brief Prints chi-squared value and degrees of freedom.
     
    cout << "chi2: " << toymatrix[i]->Get_Chi2() << endl;
    cout << "NDF: " << toymatrix[i]->Get_ndf() << endl;
}
 * @brief Saves results to ROOT file.
 *
 * Creates TTree objects, branches, and fills with data.
 
TFile *file = new TFile(Form("%d_%d.root",run_no,eve_num),"RECREATE");
TTrees and TBranches...
 * @brief Writes TTrees to file and closes file.
...
int main(int argc, char* argv[])
{
  ...
  // 	WCP2dToy::ToyMatrixMarkov toymatrix_markov(*toymatrix[end_num-1],*toymatrix[end_num],*toymatrix[end_num-1],*mergetiling[end_num-1],*mergetiling[end_num],*mergetiling[end_num-1],&allmcell);
  ...
  // 	WCP2dToy::ToyMatrixMarkov toymatrix_markov(*toymatrix[start_num+1],*toymatrix[start_num],*toymatrix[start_num+1],*mergetiling[start_num+1],*mergetiling[start_num],*mergetiling[start_num+1],&allmcell);
