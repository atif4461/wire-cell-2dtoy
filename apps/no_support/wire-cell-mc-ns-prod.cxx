 ...
 * @brief Retrieve geometry extent and print it.
 *
...
cerr << "Extent: "
     ...
     << " z:" << ex[2]/units::m << " m"
     << endl;
 * @brief Perform various operations such as simulation, clustering, and tracking.
 *
 * Performs multiple steps including simulation, clustering, and tracking.
 
... (rest of the code remains the same)
 * @brief Save results to a file.
 *
 * Saves the results of the simulation and tracking to a file.
 
TFile *file = new TFile(Form("./rootfiles/%d_%d_%d.root",run_no,subrun_no,event_no),"RECREATE");
...
...
//   // start crawler ... 
  ...
  for (auto it = cluster_list.begin();it!=cluster_list.end();it++){
    ...
    // //ncluster++;
    // WCP2dToy::ToyCrawler* toycrawler = new WCP2dToy::ToyCrawler(mscells,1,2);  // cosmic tune
    // crawlers.push_back(toycrawler);
    // WCP2dToy::ToyTracking* toytracking = new WCP2dToy::ToyTracking(*toycrawler,1); // cosmic tune
  ...
  T1->Branch("x",xx1,"x[npoints]/D");
  T1->Branch("y",yy1,"y[npoints]/D");
  T1->Branch("z",zz1,"z[npoints]/D");
  ...
  T3->Branch("x",xx1,"x[npoints]/D");
  T3->Branch("y",yy1,"y[npoints]/D");
  T3->Branch("z",zz1,"z[npoints]/D");
  ...
  T4->Branch("x",xx1,"x[npoints]/D");
  T4->Branch("y",yy1,"y[npoints]/D");
  T4->Branch("z",zz1,"z[npoints]/D");
  ...
  T5->Branch("x",xx1,"x[npoints]/D");
  T5->Branch("y",yy1,"y[npoints]/D");
  T5->Branch("z",zz1,"z[npoints]/D");
  ...
  for (int i = 0; i!=good_tracks.size();i++){
    ...
    for (int j=0;j!=npoints;j++){
      xx1[j] = track->get_centerVP().at(j).x/units::cm;
      yy1[j] = track->get_centerVP().at(j).y/units::cm;
      zz1[j] = track->get_centerVP().at(j).z/units::cm;
  ...
  for (int i=0;i!=vertices.size();i++){
    ...
    xx1[0] = vertex->Center().x/units::cm;
    yy1[0] = vertex->Center().y/units::cm;
    zz1[0] = vertex->Center().z/units::cm;
  ...
  for (int i = 0; i!=short_tracks.size();i++){
    ...
    trackid = find(all_tracks.begin(),all_tracks.end(),track) - all_tracks.begin();
    for (int j=0;j!=npoints;j++){
      xx1[j] = track->get_all_cells().at(j)->Get_Center().x/units::cm;
      yy1[j] = track->get_all_cells().at(j)->Get_Center().y/units::cm;
      zz1[j] = track->get_all_cells().at(j)->Get_Center().z/units::cm;
      theta[j] = 0;
      phi[j] = 0;
