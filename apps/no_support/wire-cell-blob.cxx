...
/**
 * Calculates the root mean square value from three input values.
 *
 ...
 * @brief Initializes arrays for storing data.
 ...
 * @brief Allocates memory for matrix objects.
 ...
 * @brief Creates metric objects.
 
WCP2dToy::ToyMetric toymetric;
WCP2dToy::BlobMetric blobmetric;
 * @brief Initializes sets and maps for storing cell data.
 ...
 * @brief Initializes variables for tracking minimum and maximum charges.
 
Double_t charge_min = 10000;
Double_t charge_max = 0;
 * @brief Loop over events and process data.
...
for (int i=start_num;i!=end_num+1;i++) {
  ...
  truthtiling[i] = new WCP2dToy::TruthToyTiling(*toytiling[i],pvv,i,gds);
     ...
     * @brief Retrieves cell selections from tiling objects.
   
  GeomCellSelection allcell = toytiling[i]->get_allcell();
  GeomCellSelection allmcell = mergetiling[i]->get_allcell();
  GeomWireSelection allmwire = mergetiling[i]->get_allwire();

     * @brief Adds cells to total cell selection.
  ...
  toymatrix[i] = new WCP2dToy::ToyMatrix(*toytiling[i],*mergetiling[i]);
     ...
     * @brief Checks solve flag and creates iterate or Markov matrix object if necessary.
   
  if (toymatrix[i]->Get_Solve_Flag()==0) {
    toymatrix_it[i] = new WCP2dToy::ToyMatrixIterate(*toymatrix[i]);
  }

     * @brief Judges simple blob reduction.
   
  toymatrix[i]->JudgeSimpleBlob(*toytiling[i],*mergetiling[i]);

     * @brief Loops over merged cells and checks containment of truth cells.
  ...
  for (int j=0;j!=allmcell.size();j++) {
     ...
     * @brief Updates charge minimum and maximum values.
 ...
 * @brief Second loop over events for processing blob data.
 
for (int i=start_num;i!=end_num+1;i++) {
     * @brief Checks simple blob reduction flag and creates SimpleBlobToyTiling object.
  ...
  if (toymatrix[i]->GetSimpleBlobReduction()) {
    if (i==start_num) {
      ...
      blobtiling[i] = new WCP2dToy::SimpleBlobToyTiling(*toytiling[i],*mergetiling[i],*toymatrix[i],*mergetiling[i-1],*toymatrix[i-1],*mergetiling[i+1],*toymatrix[i+1]);
         ...
         * @brief Saves blob cells and adds to total blob cell selection.
    ...
    for (int k=0;k!=blob_cells.size();k++) {
      total_blob_cells.push_back(blob_cells.at(k));
         ...
         * @brief Adds to blob metric object.
 ...
 * @brief Sets charge minimum and maximum values for display.
 ...
 * @brief Draws Monte Carlo data.
 
display.draw_mc(1,WCP::PointValueVector(),"colz");
 * @brief Draws slice data.
