 ...
 * reads command line arguments, and executes the main logic of the program.
 *
 ...
 *
 * This section checks if the number of command line arguments is less than 3, and if so,
 * prints the usage message and returns 1.
 ...
 *
 * This section creates several histogram objects for storing and analyzing data.
 ...
 *
 * This section configures the display settings, including color palette and contour levels.
 
gStyle->SetOptStat(0);  
 * @brief Gets GDS from geometry text file and FDS from ROOT file
 *
 * This section retrieves the geometry data source and frame data source from files.
 
WCPSst::GeomDataSource gds(argv[1]);
WCP::FrameDataSource* fds = 0;
fds = WCPSst::make_fds(root_file);
 * @brief Creates generative FDS and MicroBooNE SDS
 *
 * This section generates the generative frame data source and MicroBooNE slice data source.
 ...
 *
 * This section creates a toy event display object for visualizing data.
 
TCanvas c1("ToyMC","ToyMC",1200,600);
WCP2dToy::ToyEventDisplay display(c1,gds);
 * @brief Main loop over slices
 *
 * This section iterates over the slices, performing various operations such as tiling, matrix calculations, and plotting.
 
for (int i = 0; i!= sds.size(); i++){
 * @brief Performs tiling and matrix calculations
 *
 * This section performs tiling operations, constructs matrices, and calculates eigenvalues.
 ...
 *
 * This section fills the diagnostic histograms with calculated values.
 
eigenValHist->Fill(allcell.size(),numNonzeroEigenVals1);
 * @brief Saves output for 3D display
 *
 * This section saves the output data to a ROOT file for later visualization.
 ...
 *
 * This section prints summary statistics and ends the program.
