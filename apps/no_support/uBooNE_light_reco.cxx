...
/**
 * Destructor for uBooNE light reconstruction object 
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
...
/**
 * @brief Loads raw event data from a file and processes it for further analysis.
 *
 * This function loads the raw waveforms, timestamps, and other relevant data for both cosmic and beam events.
 * It then merges the high-gain and low-gain waveforms, reorders them, and groups the optical hits into flashes.
 * Finally, it calculates the gain and timestamp for each channel and sorts the flashes.
 *
 * @param eve_num The event number to load.
  
 * @brief Merges the raw beam waveforms and timestamps.
 *
 * This function takes the high-gain and low-gain waveforms, optical channels, and timestamps as input,
 * and merges them into a single set of waveforms and timestamps.
 *
 * @param beam_hg_wf High-gain beam waveform.
 * @param beam_hg_opch High-gain beam optical channel.
 * @param beam_hg_timestamp High-gain beam timestamp.
 * @param beam_lg_wf Low-gain beam waveform.
 * @param beam_lg_opch Low-gain beam optical channel.
 * @param beam_lg_timestamp Low-gain beam timestamp.
 * @param is_beam Flag indicating whether the event is a beam event.
 * @param opch_to_opdet Mapping from optical channel to detector.
 * @param ctr_beam Beam counter.
 
 * @brief Merges the raw cosmic waveforms and timestamps.
 *
 * This function takes the high-gain and low-gain waveforms, optical channels, and timestamps as input,
 * and merges them into a single set of waveforms and timestamps.
 *
 * @param cosmic_hg_wf High-gain cosmic waveform.
 * @param cosmic_hg_opch High-gain cosmic optical channel.
 * @param cosmic_hg_timestamp High-gain cosmic timestamp.
 * @param cosmic_lg_wf Low-gain cosmic waveform.
 * @param cosmic_lg_opch Low-gain cosmic optical channel.
 * @param cosmic_lg_timestamp Low-gain cosmic timestamp.
 * @param is_beam Flag indicating whether the event is a beam event.
 * @param opch_to_opdet Mapping from optical channel to detector.
 * @param ctr_cosmic Cosmic counter.
 
 * @brief Reorders the merged waveforms and timestamps.
 *
 * This function takes the merged beam and cosmic waveforms, optical channels, and timestamps as input,
 * and reorders them according to some criteria.
 *
 * @param fop_wf_beam Merged beam waveform.
 * @param fop_femch_beam Merged beam optical channel.
 * @param fop_timestamp_beam Merged beam timestamp.
 * @param fop_wf_cosmic Merged cosmic waveform.
 * @param fop_femch_cosmic Merged cosmic optical channel.
 * @param fop_timestamp_cosmic Merged cosmic timestamp.
 
 * @brief Groups the optical hits into flashes.
 *
 * This function takes the reordered waveforms, optical channels, and timestamps as input,
 * and groups the optical hits into flashes based on their timestamps.
 *
 * @param ophits_group Vector of vectors of optical hits.
 
 * @brief Processes the beam waveforms.
 *
 * This function takes the reordered beam waveforms and timestamps as input,
 * and applies some processing to the waveforms.
 
 * @brief Sorts the flashes.
 *
 * This function takes the vector of flashes as input,
 * and sorts them according to some criteria.
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
...
/**
 * Reorders merged waveforms from beam and cosmic events by FEM channel.
 *
 * @param[in] beam_wf        Array of beam waveforms
 * @param[in] beam_femch     Vector of FEM channels corresponding to beam waveforms
 * @param[in] beam_timestamp Vector of timestamps corresponding to beam waveforms
 * @param[in] cosmic_wf     Array of cosmic waveforms
 * @param[in] cosmic_femch  Vector of FEM channels corresponding to cosmic waveforms
 * @param[in] cosmic_timestamp Vector of timestamps corresponding to cosmic waveforms
 * This comment was generated by meta-llama/Llama-3.3-70B-Instruct:None at temperature 0.01.
*/ 
