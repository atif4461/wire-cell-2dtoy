 ...
 * processes the input files, and performs the necessary operations.
 *
...
```cpp * @brief Checks command line argument count.
 *
...
```cpp * @brief Prints usage message.
 *
...
```cpp * @brief Creates geometry data source object.
 *
 * Instantiates an object to manage geometric data.
 * @param argv Command line argument array.
 
WCPSst::GeomDataSource gds(argv[1]);
```

```cpp * @brief Retrieves geometry extent and prints it.
 *
 * Obtains the spatial extent of the geometry and displays its dimensions.
 * @return Vector of doubles representing extent.
 
std::vector<double> ex = gds.extent();
```

```cpp * @brief Opens root file containing tree structure.
 *
 * Accesses a ROOT file with a specific tree structure.
...
```cpp * @brief Retrieves trees from the file.
 *
 * Extracts trees named "TC", "Trun" from the file.
 
TTree *TC = (TTree*)file->Get("TC");
TTree *Trun = (TTree*)file->Get("Trun");
```

```cpp * @brief Sets branch addresses for the tree.
 *
 * Associates variables with branches in the tree for data retrieval.
...
```cpp * @brief Processes entries in the tree.
 *
 * Iterates through each entry in the tree, performing necessary actions.
 
for (int i=0;i!=TC->GetEntries();i++){
```

```cpp * @brief Saves all crawlers.
 *
 * Stores all crawlers in a vector for later use.
 
std::vector<WCP2dToy::ToyCrawler*> crawlers;
```

```cpp * @brief Fills trees with track data.
 *
 * Populates trees with information about tracks.
...
```cpp * @brief Writes trees to a new file.
 *
 * Saves the populated trees to a new ROOT file.
