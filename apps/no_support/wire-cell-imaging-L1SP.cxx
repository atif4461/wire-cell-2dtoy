...
/**
 * Compares two geometric wire selections based on their size and identifier values.
 *
 * @param a First geometric wire selection for comparison.
 * @param b Second geometric wire selection for comparison.
 * @return True if the first selection is greater than the second, false otherwise.
 ...
 * @brief Constructor for the ExecMon class.
 *
 * @param message Message to display when timing an operation.
 
ExecMon::ExecMon(const char* message)
 * @brief Get the execution time of an operation.
 *
 * @param message Message to display when getting the execution time.
 * @return Execution time of the operation.
 
const char* ExecMon::operator()(const char* message)
 * @brief Load geometry data from a file.
 *
 * @param filename Name of the file containing the geometry data.
...
std::vector<double> WCPSst::GeomDataSource::extent()
 * @brief Get the pitch of a wire plane.
 *
 * @param type Type of wire plane.
 * @return Pitch of the wire plane.
 
double WCPSst::GeomDataSource::pitch(WirePlaneType_t type)
 * @brief Get the angle of a wire plane.
 *
 * @param type Type of wire plane.
 * @return Angle of the wire plane.
 
double WCPSst::GeomDataSource::angle(WirePlaneType_t type)
 * @brief Initialize the singleton instance of the TPCParams class.
 *
 * @return Reference to the singleton instance.
 
TPCParams& TPCParams::Instance()
 * @brief Set the pitch of the U plane.
 *
 * @param pitch_u Pitch of the U plane.
 
void TPCParams::set_pitch_u(double pitch_u)
 * @brief Set the pitch of the V plane.
 *
 * @param pitch_v Pitch of the V plane.
 
void TPCParams::set_pitch_v(double pitch_v)
 * @brief Set the pitch of the W plane.
 *
 * @param pitch_w Pitch of the W plane.
 
void TPCParams::set_pitch_w(double pitch_w)
 * @brief Set the time slice width.
 *
 * @param ts_width Time slice width.
 
void TPCParams::set_ts_width(double ts_width)
 * @brief Get the pitch of the U plane.
 *
 * @return Pitch of the U plane.
 
double TPCParams::get_pitch_u()
 * @brief Get the pitch of the V plane.
 *
 * @return Pitch of the V plane.
 
double TPCParams::get_pitch_v()
 * @brief Get the pitch of the W plane.
 *
 * @return Pitch of the W plane.
 
double TPCParams::get_pitch_w()
 * @brief Get the time slice width.
 *
 * @return Time slice width.
 
double TPCParams::get_ts_width()
 * @brief Open a ROOT file.
 *
 * @param filename Name of the file to open.
 * @return Pointer to the opened file.
 
TFile* TFile::Open(const char* filename)
 * @brief Get a tree from a file.
 *
 * @param treename Name of the tree to retrieve.
 * @return Pointer to the retrieved tree.
 
TTree* TFile::Get(const char* treename)
 * @brief Set the branch address of a tree.
 *
 * @param branchnames Name of the branch.
 * @param addr Address of the branch.
 
void TTree::SetBranchAddress(const char* branchnames, void* addr)
 * @brief Get an entry from a tree.
 *
 * @param entry Entry number to retrieve.
 
void TTree::GetEntry(int entry)
 * @brief Create a new TH1F histogram.
 *
 * @param name Name of the histogram.
 * @return Pointer to the created histogram.
 
TH1F* TH1F::New(const char* name)
 * @brief Get the bin content of a histogram.
 *
 * @param bin Bin number to retrieve.
 * @return Content of the specified bin.
 
Double_t TH1F::GetBinContent(int bin)
 * @brief Create a new TH2F histogram.
 *
 * @param name Name of the histogram.
 * @return Pointer to the created histogram.
 
TH2F* TH2F::New(const char* name)
 * @brief Refresh the pdDataFDS object.
 *
 * @param hu_decon Deconvoluted U plane data.
 * @param hv_decon Deconvoluted V plane data.
 * @param hw_decon Deconvoluted W plane data.
 * @param eve_num Event number.
 
void WCP2dToy::pdDataFDS::refresh(TH2F* hu_decon, TH2F* hv_decon, TH2F* hw_decon, int eve_num)
 * @brief Jump to a specific event in the pdDataFDS object.
 *
 * @param eve_num Event number to jump to.
 
void WCP2dToy::pdDataFDS::jump(int eve_num)
 * @brief Get the slice data from the pdDataFDS object.
 *
 * @return Slice data.
 
WCP::Slice WCP2dToy::pdDataFDS::get()
 * @brief Get the error slice data from the pdDataFDS object.
 *
 * @return Error slice data.
 
WCP::Slice WCP2dToy::pdDataFDS::get_error()
 * @brief Add wires to the uBooNE_L1SP object.
 *
 * @param index Index of the wires to add.
 * @param wires Wires to add.
 
void WCP2dToy::uBooNE_L1SP::AddWires(int index, GeomWireSelection wires)
 * @brief Form ROIs in the uBooNE_L1SP object.
 *
 * @param size Size of the ROIs to form.
 
void WCP2dToy::uBooNE_L1SP::Form_rois(int size)
 * @brief Get the time slice set from the uBooNE_L1SP object.
 *
 * @return Time slice set.
 
std::set<int> WCP2dToy::uBooNE_L1SP::get_time_slice_set()
 * @brief Reset cells in the LowmemTiling object.
 
void WCP2dToy::LowmemTiling::reset_cells()
 * @brief Initialize bad cells in the LowmemTiling object.
 *
 * @param uplane_map Map of U plane wires.
 * @param vplane_map Map of V plane wires.
 * @param wplane_map Map of W plane wires.
 
void WCP2dToy::LowmemTiling::init_bad_cells(std::map<int, std::pair<int, int>> uplane_map, std::map<int, std::pair<int, int>> vplane_map, std::map<int, std::pair<int, int>> wplane_map)
 * @brief Check bad cells in the LowmemTiling object.
 *
 * @param prev Previous LowmemTiling object.
 * @param uplane_map Map of U plane wires.
 * @param vplane_map Map of V plane wires.
 * @param wplane_map Map of W plane wires.
 
void WCP2dToy::LowmemTiling::check_bad_cells(WCP2dToy::LowmemTiling* prev, std::map<int, std::pair<int, int>> uplane_map, std::map<int, std::pair<int, int>> vplane_map, std::map<int, std::pair<int, int>> wplane_map)
 * @brief Initialize good cells in the LowmemTiling object.
 *
 * @param slice Slice data.
 * @param slice_err Error slice data.
 * @param uplane_rms RMS values of U plane wires.
 * @param vplane_rms RMS values of V plane wires.
 * @param wplane_rms RMS values of W plane wires.
 
void WCP2dToy::LowmemTiling::init_good_cells(WCP::Slice slice, WCP::Slice slice_err, std::vector<float> uplane_rms, std::vector<float> vplane_rms, std::vector<float> wplane_rms)
 * @brief Find L1SP wires in the LowmemTiling object.
 *
 * @return Found L1SP wires.
