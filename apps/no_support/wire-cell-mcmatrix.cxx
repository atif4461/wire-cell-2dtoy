 ...
 * @brief Create geometry data source object from file path argument.
 
WCPSst::GeomDataSource gds(argv[1]);
 * @brief Get extent of geometry data source.
 
std::vector<double> ex = gds.extent();
 * @brief Print extent to standard error stream.
...
if (!fds) {
         * @brief Print error message to standard error stream if creation failed.
 ...
 * @brief Initialize variables for counting.
...
for (int i=start_num;i!=end_num+1;i++){
         * @brief Jump to current slice index.
    ...
    GeomWireSelection allmwire = mergetiling[i]->get_allwire();
         ...
         * @brief Print size of selections.
    ...
    toymatrix[i] = new WCP2dToy::ToyMatrix(*toytiling[i],*mergetiling[i]);
         ...
         * @brief Check solve flag and create iterate matrix if necessary.
 ...
 * @brief Process remaining slices and calculate metrics.
...
//...
 * @brief Close file and return program exit status.
