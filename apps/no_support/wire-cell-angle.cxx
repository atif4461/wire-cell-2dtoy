 ...
 * @brief Checks command line argument count.
 *
...
WCPSst::GeomDataSource createGeomDataSource(const char* filename)
{
  return WCPSst::GeomDataSource(filename);
}
...
std::vector<double> getExtent(WCPSst::GeomDataSource& gds)
{
  return gds.extent();
}
 * @brief Prints the extent of the geometry data source.
 *
 * @param ex Extent vector.
 
void printExtent(const std::vector<double>& ex)
{
  cerr << "Extent: "
       << " x:" << ex[0]/units::mm << " mm"
       << " y:" << ex[1]/units::m << " m"
 ...
 * @brief Performs actions based on input parameters.
 *
 * @param argc Number of command line arguments.
 * @param argv Array of command line argument strings.
 
void performActions(int argc, char* argv[])
{
...
void saveFiles(TFile* file, int eve_num, float rotate_angle)
{
  // Save files here...
...
int main(int argc, char* argv[])
{
  ...
  WCP::ToyDepositor *toydep = new WCP::ToyDepositor(fds,0,unit_dis,frame_length,x_center,y_center,z_center,rotate_angle);
