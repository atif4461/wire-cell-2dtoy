 ...
 * It reads command line arguments, initializes data sources,
 * performs deconvolution, and saves results to files.
 *
...
```
 * @brief Function to print usage message and exit the program.
 *
 ...
 *
 * @return Error code indicating incorrect usage.
 
if (argc < 3){
  cerr << "usage: wire-cell-35ton-test /path/to/celltree.root eve_num" << endl;
  return 1;
}
 * @brief Function to initialize geometry data source object.
 *
 * Creates a DetectorGDS object and sets its properties.
 *
 * @return None
...
gds.buildGDS();
 * @brief Function to set up input parameters.
 *
 * Reads command line arguments and sets up various parameters.
 *
 * @return None
...
float toffset_3=0;
 * @brief Function to perform simulation and reconstruction.
 *
 * Simulates raw waveform, performs deconvolution, and saves results.
 *
 * @return None
 ...
 * @brief Function to analyze and visualize results.
 *
 * Analyzes reconstructed data, visualizes results, and saves them to files.
 *
 * @return None
...
WCPSst::ToyuBooNESliceDataSource sds_th(gds,st_fds,st_fds,500, 
                        ...
                        nwire_u, 
                        nwire_v, nwire_w); 
 * @brief Function to finalize and clean up resources.
 *
 * Finalizes analysis, cleans up memory, and closes files.
 *
 * @return Program exit status.
