 ...
 * @brief Set up geometry data source object.
 *
 * Creates a WCPSst::GeomDataSource object using the path to ChannelWireGeometry.txt file.
 *
 * @param argv Array of command line argument strings.
 
void setupGeometry(char* argv[])
{
  WCPSst::GeomDataSource gds(argv[1]);
}
 * @brief Calculate extent of geometry data source.
 *
 * Gets the extent of the geometry from the data source and prints it.
 *
 * @param gds Geometry data source object.
 
void calculateExtent(WCPSst::GeomDataSource gds)
{
  std::vector<double> ex = gds.extent();
  cerr << "Extent: "
       << " x:" << ex[0]/units::mm << " mm"
 ...
 *
 * Performs the reconstruction process using the provided data and parameters.
 *
 * @param argc Number of command line arguments.
 * @param argv Array of command line argument strings.
 
void performReconstruction(int argc, char* argv[])
{
  //... (rest of the reconstruction code)
}
 * @brief Save results.
 *
 * Saves the reconstructed data to a file.
 *
 * @param argc Number of command line arguments.
 * @param argv Array of command line argument strings.
 
void saveResults(int argc, char* argv[])
{
  //... (code to save results)
}
 * @brief Main program body.
 *
 * Calls functions to perform the reconstruction process.
 *
 * @param argc Number of command line arguments.
 * @param argv Array of command line argument strings.
 * @return Program exit status.
...
int mainBody(int argc, char* argv[]) 
{
  ...
  setupGeometry(argv);
  calculateExtent(gds);
  performReconstruction(argc, argv);
  saveResults(argc, argv);
  return 0;
}
