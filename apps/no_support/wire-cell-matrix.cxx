...
cerr << "Extent: "
     ...
     << " z:" << ex[2]/units::m << " m"
     << endl;
 * @brief Sets root file path and tree path.
...
if (!fds) {
     * @brief Prints error message to standard error stream.
   
  cerr << "ERROR: failed to get FDS from " << root_file << endl;
  return 1;
}
 * @brief Creates toy depositor object.
 ...
 * @brief Initializes variables for loop control.
 ...
 * @brief Loop over events in slice data source.
 
for (int i=start_num;i!=end_num+1;i++){
     * @brief Jumps to current event in slice data source.
   
  sds.jump(i);

     * @brief Retrieves slice object from slice data source.
   
  WCP::Slice slice = sds.get();

     * @brief Creates tiling objects for current event.
   
  toytiling[i] = new WCP2dToy::ToyTiling(slice,gds);
  mergetiling[i] = new WCP2dToy::MergeToyTiling(*toytiling[i],i);

     * @brief Retrieves selections of cells and wires from tiling objects.
  ...
  GeomWireSelection allmwire = mergetiling[i]->get_allwire();
     ...
     * @brief Prints size of selections to standard output stream.
   
  cout << i << " " << allmcell.size() << " " << allmwire.size() << endl;

     * @brief Creates truth tiling object for current event.
   
  truthtiling[i] = new WCP2dToy::TruthToyTiling(*toytiling[i],pvv,i,gds);

     * @brief Creates matrix object for current event.
   
  toymatrix[i] = new WCP2dToy::ToyMatrix(*toytiling[i],*mergetiling[i]);

     * @brief Checks solve flag of matrix object.
   
  if (toymatrix[i]->Get_Solve_Flag()==0)
    toymatrix_it[i] = new WCP2dToy::ToyMatrixIterate(*toymatrix[i]);

     * @brief Prints chi-squared and degrees of freedom to standard output stream.
   
  cout << "chi2: " << toymatrix[i]->Get_Chi2() << endl;
  cout << "NDF: " << toymatrix[i]->Get_ndf() << endl;
}
 * @brief Prints metric object.
